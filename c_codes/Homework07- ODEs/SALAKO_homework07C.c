...
int main(void)
{
	...
	double upper_limit = 30000.0; //depth in km
	double lower_limit = 0.0; // depth in km
	...
	while(print_runge)
	{
		...
		fprintf(stdout, "Runge-Kutta(NumberOfSteps: %d) f(x=30000.0) = %.5f\n", number_of_steps, rk_current_func);
...
// Heat Equation:
double S(z, f_z, g_z)
double z; // the value of x
double f_z; // the function of x
double g_z; // The first derivative of x
{
	double H_z; 
	double dx2; // The second derivative...

	H_z = (4*1e-6) * pow(M_E, -(z/10000));
	dx2 = -H_z / 4;
	...
	
{
	...
	FILE*fpin;
	fpin = fopen("SALAKO.HW07C.output", "w");
    ...
    //The initial condition, which is the boundary condition:
    // initial_f = T(z=0.0)=2.0 and initial_g = dT/dz(z=0.0) =0.0175
	double initial_f = 20.0, initial_g = 0.0175;
